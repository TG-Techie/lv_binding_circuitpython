...
def get_ctor(obj_name):
    global obj_ctors
    return next(
        ctor for ctor in obj_ctors if ctor.name == ctor_name_from_obj_name(obj_name)
    )
...
STATIC inline mp_obj_t lv_to_mp(LV_OBJ_T *lv_obj)
{
    ...
    if (!self)
    {
        ...
        const lv_obj_class_t *const lv_obj_cls = lv_obj_get_class(lv_obj);
...
def try_generate_struct(struct_name, struct):
    ...
    for decl in flatten_struct_decls:
        ...
        if callback:
            ...
            write_cases.append(
                "case MP_QSTR_{field}: data->{field} = {cast}mp_lv_callback(dest[1], {lv_callback} ,MP_QSTR_{struct_name}_{field}, {user_data}, NULL, NULL, NULL); break; // converting to callback {type_name}".format(
